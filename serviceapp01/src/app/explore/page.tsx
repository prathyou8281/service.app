...
export default function ExploreServices() {
  ...
  useEffect(() => {
    ...
    // Fetch ACTUAL services from DB (not just types)
    fetch(`${API_BASE_URL}/services`)
      .then(res => res.json())
      .then(data => {
        const allServices = Array.isArray(data) ? data : (data.data || []);

        // Deduplicate services by name to show unique offerings
        const uniqueServices = Array.from(new Map(allServices.map((item: any) => [item.name, item])).values());
  ...
  const getIcon = (name: string) => {
    ...
    if (lower.includes('web') || lower.includes('app')) return Monitor;
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
        ...
        {loading ? (
          ...
          <div className="text-center py-24 card border-dashed">
            ...
            <p className="text-[var(--muted)] font-bold">No services active at this time.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {services.map((service, idx) => {
              ...
              return (
                <motion.div
                  key={service.id} // Use ID of the representative service
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: idx * 0.05 }}
                  viewport={{ once: true }}
                  onClick={() => router.push(`/services/vendors?name=${encodeURIComponent(service.name)}`)}
                ...
                >
                  <div>
                    ...
                    <h3 className="text-xl font-black mb-3 text-slate-900 group-hover:text-blue-600 transition-colors">{service.name}</h3>
                    <p className="text-sm font-medium text-[var(--muted)] leading-relaxed mb-6 line-clamp-3">
                      {service.short_description || service.description || `Premium ${service.name.toLowerCase()} solutions by top-rated industry specialists.`}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-[var(--border)]">
                    ...
                    <button className="flex items-center gap-2 text-sm font-black text-blue-600 group-hover:translate-x-1 transition-transform">
                      {user ? "Book Now" : "Explore"} <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>
                </motion.div>
              );
